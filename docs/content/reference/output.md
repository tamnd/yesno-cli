...
describes what users get. Pick a format with `-o`, or let yesno choose:
a table when writing to a terminal, JSONL when piped.

## Formats

```bash
yesno <command> -o table   # aligned columns for reading
yesno <command> -o jsonl   # one JSON object per line, for piping
yesno <command> -o json    # a single JSON array
yesno <command> -o csv     # spreadsheet friendly
yesno <command> -o tsv     # tab-separated
yesno <command> -o url     # just the URL column
yesno <command> -o raw     # the underlying bytes, unformatted
```
...
```bash
yesno <command> --fields id,title,url
```
...
```bash
yesno <command> --template '{{.URL}} {{.Title}}'
```
...
```bash
yesno <command>            # a table, because this is a terminal
yesno <command> | wc -l    # JSONL, because this is a pipe
```
