// Package cli assembles the yesno command tree from the yesno
...
// NewApp assembles the kit application from the yesno domain. The
...
//
// To add a command, declare it in yesno/domain.go with kit.Handle and it
// appears here automatically. Reach for app.AddCommand only for a verb that does
// not fit the emit-records shape, the way version does below.
func NewApp() *kit.App {
	id := yesno.Domain{}.Info().Identity
	id.Version = Version

	app := kit.New(id)
	(yesno.Domain{}).Register(app)
